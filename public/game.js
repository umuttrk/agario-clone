...
socket.on('place-me-at-all-clients-screen', data => {
    $('#parent').append(' <div class="character" id=' + data.id + ' style="  width:' + element.size + 'px; height:' + element.size + 'px;  transform: translate(' + data.x + 'px, ' + data.y + 'px); background-color: rgb(' + data.r + ', ' + data.g + ', ' + data.b + ');"></div>');
})
